...
\title{Seed geometry predicts the onset of dissipative boundary transport in a spherical Gray--Scott system}

\author{{\L}ukasz Ciesielski}
\affiliation{Independent Researcher}
\email{lukasz@ciesiel.ski}

\date{\today}

\begin{abstract}
We investigate how the spatial geometry of initial seeding controls the first-passage statistics of structure transport to the boundary in a three-dimensional Gray--Scott reaction--diffusion system on a spherical domain with no-flux boundary conditions. Three seed topologies of equal volume---central concentration, sub-boundary shell, and space-filling distribution---are systematically compared using pre-registered acceptance criteria and Kaplan--Meier survival analysis of boundary dissipation onset times $\tb$.
Across two grid resolutions ($192^3$ and $288^3$, combined $n=28$ per topology) we find that the ordering distributed $<$ shell $<$ central is preserved with median $\tb$ ratios spanning nearly one decade, stable across threshold choices of \SIrange{1}{50}{\percent} and yielding log-rank $p_{\mathrm{Holm}} \le 1.3\times10^{-6}$ for all pairwise contrasts on both scales. Reproducibility was verified by independently seeded batches ($n=6+6$) run two weeks apart on identical parameters, yielding median $\tb$ differences below \SI{1}{\percent} and confirming session-to-session stability. The scale ratio $\tau(288^3)/\tau(192^3)=1.40$ for shell falls within pre-registered transport bounds $[1.25,1.75]$, consistent with transport-limited scaling. Comparison against a parametric sensitivity yardstick $(F,k)\pm\SI{0.5}{\percent}$ in the working regime shows topological effects exceeding parametric noise by factors $3.1\times$ and $4.7\times$ for the two dominant contrasts, and $1.57\times$ for the shell--distributed contrast---the latter at the boundary of methodological acceptability.
A decomposition of the onset time into a pattern-activation phase and a subsequent transport phase indicates that the ordering reflects at least two distinct geometric mechanisms rather than radial distance alone.
Results establish the geometry of dissipative seeding as a quantitative predictor of temporal transport statistics within Prigogine's dissipative structure framework, complementing existing thermodynamic descriptions of pattern formation.
\end{abstract}
...
Here we combine both threads. We study the three-dimensional Gray--Scott system~\cite{pearson1993,gray1984} on a spherical domain with no-flux boundary conditions and ask whether the topology of initial seeding---central concentration, sub-boundary shell, or space-filling distribution, at equal seeded volume---predicts the first-passage statistics of boundary dissipation onset, $\tb$. Using pre-registered acceptance criteria, Kaplan--Meier survival analysis with right censoring, and a parametric sensitivity yardstick separating topological effects from working-point sensitivity, we find that seed geometry robustly orders the transport statistics across two grid resolutions, with a scale ratio consistent with transport-limited scaling. The protocol, including frozen evaluation criteria and pre-registered scaling bounds, was fixed before the confirmation runs; all raw data, analysis scripts, and the decision audit trail are publicly available.

The paper is organized as follows. Section~\ref{sec:methods} defines the model, the seeding topologies, and the observable $\tb$. Section~\ref{sec:protocol} describes the pre-registered confirmation protocol. Section~\ref{sec:results} presents the results across both grid scales, including the sensitivity yardstick. Section~\ref{sec:discussion} discusses interpretation, limitations, and directions for extending the thermodynamic characterization.
...
Against this yardstick, the inter-topology differences at $192^3$ are 2970 (central${-}$distributed, $4.7\times$), 1980 (central${-}$shell, $3.1\times$), and 990 (shell${-}$distributed, $1.57\times$); see Table~\ref{tab:yardstick}. The two dominant contrasts exceed working-point sensitivity by clear margins; the shell--distributed contrast lies at the boundary of methodological acceptability and is flagged as such. We emphasize that all three contrasts remain individually significant under log-rank at both scales; the yardstick comparison addresses a distinct question---whether topology outweighs parameter uncertainty---and receives a correspondingly nuanced answer.

\subsection{Decomposition of the onset time}
\label{sec:decomposition}

The scale ratios above test whether the ordering is transport-limited, but $\tb$ itself is a composite: structure must first develop a mature pattern before boundary export can begin. To separate these phases we use the fill fraction $\phi(t)$---the fraction of the domain with $v>0.1$---as an auxiliary observable. Because all topologies seed at equal volume, $\phi(0)\approx0.057$ for all three; we therefore work with the normalized fill $\tilde\phi(t) = [\phi(t)-\phi(0)]/[1-\phi(0)]$, which vanishes at $t=0$ by construction and measures growth into the initially unstructured domain.
...
Five limitations deserve explicit statement. First, the shell--distributed contrast (990 steps) exceeds the parametric sensitivity yardstick (630 steps) by only $1.57\times$; while log-rank significance is unambiguous at both scales, the claim that geometry \textit{dominates} working-point uncertainty is strong only for the contrasts involving the central topology. Second, arm~C confounds domain scale with seeded fraction (\SI{5.7}{\percent} $\to$ \SI{3.8}{\percent}); the fraction-preserving control exists only for the shell topology (arm~$\mathrm{B}'$), and the anomalous distributed ratio ($2.25$) is interpreted, but not isolated, as a fraction effect. Third, the sensitivity yardstick rests on a single seed and single topology; a multi-seed, multi-topology yardstick would sharpen the comparison. Fourth, all results pertain to one working point ($F=0.042$, $k=0.062$) of one model; the strong $k$-dependence observed in the probe suggests the transport picture should be re-examined across the Gray--Scott phase diagram before any claim of generality. Fifth, $\tb$ is a composite observable: the decomposition of Sec.~\ref{sec:decomposition} shows it mixes a pattern-activation phase, plausibly governed by seed surface-to-volume, with a subsequent transport phase, and these order the topologies differently. The transport proxy relies on combining two observables measured on different fields, and a rigorous separation requires front-position tracking not recorded in the present runs. The reported ordering is robust and reproducible, but its mechanistic attribution to ``transport'' in the strict sense is therefore provisional.
...
In a three-dimensional Gray--Scott system on a spherical domain, the spatial geometry of equal-volume initial seeding predicts the first-passage statistics of boundary dissipation onset. The ordering distributed $<$ shell $<$ central spans nearly a decade in median $\tb$, replicates across two grid resolutions and independent seed batches, is stable across threshold choices, and scales---where seeding is self-similar---within pre-registered front-propagation bounds. Two of three pairwise contrasts exceed parametric working-point sensitivity by factors of \numrange{3}{5}; the third lies at the acceptability boundary and is reported as such. Within Prigogine's framework, these results identify seed geometry as a quantitative control parameter for the transient opening of a dissipative system's export channel.
...
The recording horizon scales with grid size (CRITERIA~v2 \S7): records are taken every 500 integration steps, giving a horizon of $T=8100$ recorded units at $192^3$ (45\,000 steps) and $T=12\,060$ at $288^3$ (67\,000 steps). The scaling keeps the horizon proportional to the domain radius, so front-propagating structure has commensurate time to reach the boundary at both resolutions. A run whose boundary dissipation never reaches threshold within $T$ is right-censored rather than discarded.
